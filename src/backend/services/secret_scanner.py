...
class SecretScanner:
    ...
    def __init__(self):
        ...
        self.patterns: Dict[SecretType, List[tuple[Pattern[str], float]]] = {
            ...
            SecretType.AWS_SECRET_KEY: [
                ...
                (
                    re.compile(
                        r'(?i)(?:aws_secret_access_key|aws_secret_key|secret_access_key)\s*[=:]\s*["\']?([0-9a-zA-Z/+]{40})["\']?'
                    ),
                    0.6,
                ),
            ],
            ...
            SecretType.SSH_PRIVATE_KEY: [
                (
                    re.compile(r"-----BEGIN (OPENSSH |EC|DSA|RSA) PRIVATE KEY-----"),
                    0.95,
                ),
            ],
            ...
            SecretType.API_KEY: [
                (
                    re.compile(
                        r'[aA][pP][iI][_]?[kK][eE][yY][ _]?[=:]? [\'"]([a-zA-Z0-9_-]{20,})[\'"]'
                    ),
                    0.7,
                ),
                (
                    re.compile(
                        r'[aA][pP][iI][_]?[tT][oO][kK][eE][nN][ _]?[=:]? [\'"]([a-zA-Z0-9_-]{20,})[\'"]'
                    ),
                    0.7,
                ),
                (
                    re.compile(
                        r'[sS][eE][cC][rR][eE][tT][ _]?[=:]? [\'"]([a-zA-Z0-9_-]{20,})[\'"]'
                    ),
                    0.7,
                ),
            ],
        ...
        self.ignore_patterns = [
            ...
            re.compile(
                r"<[^>]+>|\[[^\]]+\]|{[^}]+}"
            ),  # HTML/XML tags, markdown links, template placeholders
        ]
    ...
    def _calculate_confidence(
        ...
        if secret_type == SecretType.AWS_SECRET_KEY:
            ...
            if len(matched_text) == 40 and re.match(
                r"^[0-9a-zA-Z/+]{40}$", matched_text
            ):
    ...
    return [
        {
            ...
            "context": match.context.replace(match.value, match.masked_value),
