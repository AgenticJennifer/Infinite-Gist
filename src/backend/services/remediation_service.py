...
Handles the lifecycle of remediation actions: make_private, delete, and rotate.
...
class RemediationService:
    ...
    async def delete_gist(self, finding_id: int, user_id: int) -> RemediationAction:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to commit error state: {commit_err}")
                try:
                    self.db.rollback()
                except Exception:
                    pass
    ...
    async def get_action_status(
        ...
        """
        Get current status of a remediation action owned by the given user.

        Args:
            action_id: The ID of the action
            user_id: The requesting user's ID; the action is only returned if
                it belongs to this user

        Returns:
            RemediationAction or None if not found (or not owned by user_id)
        """
        return (
