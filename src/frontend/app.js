...
async function renderFindings() {
  ...
  async function loadFindings() {
    ...
    try {
      ...
      const ariaSort = (col) => sortCol !== col ? 'none' : sortDir === 'asc' ? 'ascending' : 'descending';
      table.innerHTML = `
        <thead>
          <tr>
            <th class="sortable" data-col="severity" role="button" tabindex="0" aria-sort="${ariaSort('severity')}">Severity${sortIcon('severity')}</th>
            <th class="sortable" data-col="secret_type" role="button" tabindex="0" aria-sort="${ariaSort('secret_type')}">Type${sortIcon('secret_type')}</th>
            <th class="sortable" data-col="file_path" role="button" tabindex="0" aria-sort="${ariaSort('file_path')}">File${sortIcon('file_path')}</th>
            <th class="sortable" data-col="confidence" role="button" tabindex="0" aria-sort="${ariaSort('confidence')}">Confidence${sortIcon('confidence')}</th>
            <th class="sortable" data-col="status" role="button" tabindex="0" aria-sort="${ariaSort('status')}">Status${sortIcon('status')}</th>
            <th class="sortable" data-col="detected_at" role="button" tabindex="0" aria-sort="${ariaSort('detected_at')}">Detected${sortIcon('detected_at')}</th>
          </tr>
        </thead>
        <tbody></tbody>`;
      ...
      data.forEach(f => {
        ...
        tr.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); goToFinding(); } };
      ...
      table.querySelectorAll('th.sortable').forEach(th => {
        ...
        th.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); applySort(); } };
      });
    } catch (e) {
      tableContainer.innerHTML = '';
